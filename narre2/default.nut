...
//Water Bird (Finish the swimming challenge)
if (!("achievement_water_bird" in state)) {
 state.achievement_water_bird <- false;
}
...
//Underground, level A Dark Tunnel, secret area 1
if (!("w4_ldarktunnel_s1" in state)) {
 state.w4_ldarktunnel_s1 <- false;
}
...
//Mountains, level Penguins Don't Fly (Don't Fall to the Valley), secret area 1
if (!("w5_ldontvalley_s1" in state)) {
 state.w5_ldontvalley_s1 <- false;
}

//Mountains, level Snowy Peaks, secret area 1
if (!("w5_lsnowypeaks_s1" in state)) {
 state.w5_lsnowypeaks_s1 <- false;
}
...
//Rocks, level Rock Solid, secret area 1
if (!("w8_lrocksolid_s1" in state)) {
 state.w8_lrocksolid_s1 <- false;
}

//Rocks, level Rock Solid, secret area 2
if (!("w8_lrocksolid_s2" in state)) {
 state.w8_lrocksolid_s2 <- false;
}
...
//Swamp, level Show Me What I Missed, secret area 1
if (!("w10_lshowme_s1" in state)) {
 state.w10_lshowme_s1 <- false;
}

//Swamp, level Muddiving, secret area 1
if (!("w10_lmuddive_s1" in state)) {
 state.w10_lmuddive_s1 <- false;
}

//Swamp, level A Wisp Overload, secret area 1
if (!("w10_loverwisp_s1" in state)) {
 state.w10_loverwisp_s1 <- false;
}

//Swamp, level A Muddy Swampland, secret area 1
if (!("w10_lmuddyswamp_s1" in state)) {
 state.w10_lmuddyswamp_s1 <- false;
}
...
//Volcano, level Warmth from the Core, secret area 1
if (!("w12_lwarmcore_s1" in state)) {
 state.w12_lwarmcore_s1 <- false;
}
...
//River, level Wet Water, secret area 1
if (!("w13_lwetwater_s1" in state)) {
 state.w13_lwetwater_s1 <- false;
}

//River, level Wet Water, secret area 2
if (!("w13_lwetwater_s2" in state)) {
 state.w13_lwetwater_s2 <- false;
}
...
//StoneCave, level Stone Zone, secret area 1
if (!("w14_lstonezone_s1" in state)) {
 state.w14_lstonezone_s1 <- false;
}
...
//Meadows, level Land of Hay, secret area 1
if (!("w15_llandhay_s1" in state)) {
 state.w15_llandhay_s1 <- false;
}
...
//Geysers, level Land of Hot, secret area 1
if (!("w16_llandhot_s1" in state)) {
 state.w16_llandhot_s1 <- false;
}
...
//Fortress, level Traps, secret area 1
if (!("w17_ltraps_s1" in state)) {
 state.w17_ltraps_s1 <- false;
}
...
//Matrix, level Digital Insanity, secret area 1
if (!("w18_ldiginsane_s1" in state)) {
 state.w18_ldiginsane_s1 <- false;
}
